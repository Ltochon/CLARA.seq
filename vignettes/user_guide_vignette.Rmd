...
# Installation
This package is still in a test phase. Therefore it is only available on GitHub. The link to the repository is : https://github.com/Ltochon/CLARA.seq

To download the package and get the functions in your personnal environnement, here are the commands :
...
To begin the creation of sequences to cluster is needed. The use of the TraMineR package allows to get example datas :
...
With the sequences previously created, the clustering can be applied on them. The process will be parallelized to find the best clustering faster. 
The number of cores used is initially set to use all possible cores minus one to avoid any overloads. 
  
The most important parameters are the number of clusters (nb_cluster), the number of subsets to use (nb_sample) and the size of each one of thes subsets (size_sample). 
...
The function calls is as follow for CLARA with the mean distance :

```R
my_cluster <- clara_clust(mvad.seq,nb_cluster = 4, nb_sample = 20, 
+   size_sample = 20, with.diss = TRUE)
```
A console output will be written with a summary of each iterations mean distance value :
...
The plot parameter determines if the mean distance value's evolution must be shown at the end.

The graph is the representation of the value evolution for each iteration (depending of the method used) :
```{r , echo=FALSE, out.width = '39%',fig.align='center',fig.show='hold'}
knitr::include_graphics("C:/Projet_Bachelor/projet-de-bachelor/img_doc/dist_graph.png")
```
In case of the user want to get this plot again, the default function *plot()* can render this plot again thanks to the function
plot.clara-seq which detect if the object passed as parameter is from the clara-seq's class. 
...
The call of this function is similar as the one previously seen. The only difference is that the *find_best_method* must be specified as *DB*.
In this example, we are using the same sequences creater previously.
```R
my_cluster <- clara_clust(mvad.seq,nb_cluster = 4, nb_sample = 20, 
...
The same object of the same class than with the previous method will be created and the same type of plot can be rendered at any time with the same function :
...
### Research of the best clutering
...
+   plot = TRUE)
```
The main parameters are the number of cluster (nb_cluster), the number of neighbours to explore (maxneighbours) and the number of tries to dp (numlocal) i.e. the number of time that we will initialise a set of medoids and visit *maxneighbours* neighbours. 
...
```
The main parameters are the number cluster, the number of subsets (nb_sample) and the size of each one of them (size_sample) as in the clara_clust() function, the threshold to exclude outliers (threshold), the maximal number of iterations to find the best set of medoids (max_iter) and the number of candidate to test to find a better medoid (p). 
...
## Quality analysis
A quality analysis can be execute with the help of the Davies-Bouldin function and the *TraMineR* package.  
For the following examples, the object *my_cluster* coming from the previous CLARA clusterin is used but the quality analysis is working with every object from each functions.

### Davies-Bouldin Index

Davies-Bouldin provides a quality analysis index working with big datasets because it doen't require to compute the complete distance matrix. Indeed, this index aims to calculate the following equation :
\begin{align*}
...
The call of this function will produce a console output as follow :
...
With the package *TraMineR* it is possible to draw a graph with the sequence distribution. To do this, the function *seqdplot* will be use as follow : 

```R
seqdplot(mvad.seq, my_cluster$clusters)
```
A graph as the following can be obtained :
